...
async function getWeatherForecast(latLong) {
  ...
  let response = await fetch(
    `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${long}&exclude=minutely&appid=${openWeatherAPIKey}`,
    { mode: 'cors' },
  );
  try {
...
async function getOpenWeatherForCity() {
  // const latLong = await getLatLongPositionStack(); // positionStack for testing
  const latLong = await getLatLong(); // google API
